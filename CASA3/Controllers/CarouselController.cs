    ...
    public class CarouselController : Controller
    {
        ...
        public async Task<IActionResult> CreateCarousel([FromForm] CarouselDto model)
        {
            try
            {
                ...
                var imageExt = Path.GetExtension(model.BackgroundImage.FileName).ToLowerInvariant();
                ...
                var backgroundImageUrl = await _cloudinaryService.UploadImageAsync(model.BackgroundImage, "casa3/carousel-images");
                if (string.IsNullOrEmpty(backgroundImageUrl))
                    return Json(new { success = false, message = "Failed to upload background image." });

                // Upload brochure (Home only)
                ...
                if (model.PageType == CarouselPageType.Home && model.Brochure != null && model.Brochure.Length > 0)
                {
                    ...
                    if (!string.Equals(Path.GetExtension(model.Brochure.FileName), ".pdf", StringComparison.OrdinalIgnoreCase))
                        return Json(new { success = false, message = "Brochure must be a PDF file." });

                    brochureUrl = await _cloudinaryService.UploadRawAsync(model.Brochure, "casa3/carousel-brochures");
        ...
        public async Task<IActionResult> UpdateCarousel([FromForm] CarouselUpdateDto model)
        {
            try
            {
                ...
                if (model.BackgroundImage != null && model.BackgroundImage.Length > 0)
                {
                    ...
                    backgroundImageUrl = await _cloudinaryService.UploadImageAsync(model.BackgroundImage, "casa3/carousel-images");
                    if (string.IsNullOrEmpty(backgroundImageUrl))
                        return Json(new { success = false, message = "Failed to upload background image." });
                }

                // Upload new brochure if provided (Home only)
                if (model.PageType == CarouselPageType.Home && model.Brochure != null && model.Brochure.Length > 0)
                {
                    if (model.Brochure.Length > 10 * 1024 * 1024)
                        return Json(new { success = false, message = "Brochure file size exceeds 10MB limit." });
